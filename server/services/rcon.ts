...
export class RconManager extends EventEmitter {
  ...
  private async connect(connectionId: string): Promise<boolean> {
    ...
    try {
      ...
      if (isSuccess) {
        ...
        console.log(`RCON connected successfully to ${connection.host}:${connection.port}`);
      ...
      console.error(`RCON connection failed (attempt ${connection.connectionAttempts}):`, error);
  ...
  private async processNextCommand(): Promise<void> {
    ...
    try {
      ...
      console.log(`RCON command executed: ${command.command} ${command.args.join(' ')} -> ${result}`);
    } catch (error) {
  ...
  public async queueCommand(
    ...
    const insertIndex = this.commandQueue.findIndex(cmd => cmd.priority < priority);
    if (insertIndex === -1) {
  ...
  public async executeImmediate(
    ...
    try {
      ...
      rconCommand.error = error instanceof Error ? error.message : 'Unknown error';
      throw error;
    }
  }
...
export const RconCommands = {
  give: (username: string, item: string, quantity: number = 1, nbt?: string) => {
  ...
  setRank: (username: string, rank: string) => {
    return rconManager.queueCommand('lp', ['user', username, 'parent', 'set', rank], 10);
  },

  giveCoins: (username: string, amount: number) => {
    return rconManager.queueCommand('eco', ['give', username, amount.toString()], 5);
  },
  ...
  sendTitle: (username: string, title: string, subtitle?: string) => {
    ...
    if (subtitle) {
      commands.push(rconManager.queueCommand('title', [username, 'subtitle', subtitle], 3));
    }
    return commands;
  },
