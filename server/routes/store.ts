...
const convertToApiFormat = (item: any): StoreItem => ({
  ...
  commands:
    item.deliveryCommands?.map(
      (cmd: any) => `${cmd.command} ${cmd.args.join(" ")}`,
    ) || [],
  metadata: {
...
export const createPurchase: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    setTimeout(
      async () => {
        const purchaseIndex = mockPurchases.findIndex(
          (p) => p.id === purchase.id,
        );
        if (purchaseIndex !== -1) {
          ...
          try {
            ...
            if (deliveryResult.success) {
              console.log(
                `✅ Instant delivery completed for ${playerUsername} (${itemId})`,
              );
            } else {
              console.error(
                `❌ Delivery failed for ${playerUsername} (${itemId}): ${deliveryResult.error}`,
              );
              mockPurchases[purchaseIndex].status = "failed";
            }
          } catch (deliveryError) {
            console.error(
              `Delivery error for ${playerUsername} (${itemId}):`,
              deliveryError,
            );
            mockPurchases[purchaseIndex].status = "failed";
          }
    ...
    ); // 2-4 seconds for realism

    const response: ApiResponse<Purchase> = {
      success: true,
      data: purchase,
      message:
        "Purchase created successfully. Processing payment and delivery...",
    };
    ...
    const response: ApiResponse = {
      success: false,
      error:
        error instanceof Error ? error.message : "Purchase processing failed",
    };
...
export const retryDelivery: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (deliveryResult.success) {
      ...
      mockPurchases[purchaseIndex].deliveredAt = new Date().toISOString();
      console.log(
        `✅ Retry delivery successful for ${playerUsername} (${purchase.itemId})`,
      );
    } else {
      mockPurchases[purchaseIndex].status = "failed";
      mockPurchases[purchaseIndex].failureReason = deliveryResult.error;
      console.error(
        `❌ Retry delivery failed for ${playerUsername} (${purchase.itemId}): ${deliveryResult.error}`,
      );
    }
