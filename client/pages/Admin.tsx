...
export default function Admin() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'queued': return <Clock className="h-4 w-4 text-primary-blue" />;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'processing': return 'text-gold border-gold';
      case 'queued': return 'text-primary-blue border-primary-blue';
      case 'failed': return 'text-redstone border-redstone';
  ...
  if (!auth.isAuthenticated) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <p className="text-muted-foreground mb-8">You need to be logged in as an administrator to access this page.</p>
      </div>
    );
  }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">Server management and monitoring</p>
          </div>
          <div className="flex items-center gap-4">
            <Badge className={`${serverStats?.rconConnected ? 'bg-neon-green/20 text-neon-green border-neon-green/30' : 'bg-redstone/20 text-redstone border-redstone/30'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card className="gaming-card">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-primary-blue">
                {serverStats?.onlinePlayers || 0}/{serverStats?.maxPlayers || 100}
              </div>
              <p className="text-xs text-muted-foreground">
                {((serverStats?.onlinePlayers || 0) / (serverStats?.maxPlayers || 100) * 100).toFixed(1)}% capacity
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="gaming-card">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {(serverStats?.tps || 0) >= 19 ? 'Excellent' : 'Needs attention'}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="gaming-card">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-primary-purple">
                {purchaseStats ? ((purchaseStats.completed / purchaseStats.total) * 100).toFixed(1) : 0}%
              </div>
              <p className="text-xs text-muted-foreground">
        ...
        <Tabs defaultValue="delivery" className="w-full">
          <TabsList className="grid w-full grid-cols-4 mb-8 bg-card/50">
            <TabsTrigger value="delivery" className="data-[state=active]:bg-primary-blue data-[state=active]:text-white">
              <Zap className="mr-2 h-4 w-4" />
              Delivery Queue
            </TabsTrigger>
            <TabsTrigger value="rcon" className="data-[state=active]:bg-primary-purple data-[state=active]:text-white">
              <Settings className="mr-2 h-4 w-4" />
              RCON Console
            </TabsTrigger>
            <TabsTrigger value="purchases" className="data-[state=active]:bg-gold data-[state=active]:text-white">
              <BarChart3 className="mr-2 h-4 w-4" />
              Purchases
            </TabsTrigger>
            <TabsTrigger value="analytics" className="data-[state=active]:bg-emerald data-[state=active]:text-white">
                    ...
                    deliveryQueue.map((item) => (
                      <div key={item.id} className="flex items-center justify-between p-4 border border-border rounded-lg">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="font-medium">{item.playerUsername}</div>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {rconLoading ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}
                    </Button>
                  </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-neon-green">{purchaseStats?.completed || 0}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm">Pending</span>
                      <span className="font-semibold text-gold">{purchaseStats?.pending || 0}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm">Failed</span>
                      <span className="font-semibold text-redstone">{purchaseStats?.failed || 0}</span>
                    </div>
                    <div className="border-t pt-4">
                      <div className="flex justify-between">
                        <span className="font-medium">Total Revenue</span>
                        <span className="font-bold text-gold">₹{purchaseStats?.revenue.toLocaleString() || 0}</span>
                      </div>
                    </div>
                  </div>
                </CardContent>
          ...
          <TabsContent value="analytics" className="space-y-6">
            <Alert>
              ...
              <AlertDescription>
                Analytics and reporting features are coming soon. This will include sales charts, player engagement metrics, and performance analytics.
              </AlertDescription>
            </Alert>
          </TabsContent>
        </Tabs>
